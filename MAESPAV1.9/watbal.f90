...
SUBROUTINE INITWATBAL(LAYTHICK,WETTINGBOT,WETTINGTOP, &
                            ...
                            SOILDATA,USEMEASSW,IWATTABLAYER,&
                            ISIMWATTAB)
...
!**********************************************************************
    ...
    ELSE
        WSOIL = 0.
        WSOILROOT = 0.
        DO I = 1,MIN(NLAYER,IWATTABLAYER-1)
            WSOIL = WSOIL + INITWATER(I)*LAYTHICK(I)*1000
        END DO
    ...
    IF(SOILDATA.GT.0.AND.USEMEASSW.EQ.0)THEN
        USEMEASSW = 1
        CALL SUBERROR('WARNING: USEMEASSW is set to 1, measured soil water is used.',&
                        IWARN,IOERROR)
    ENDIF
...
!**********************************************************************
      ...
      SUBROUTINE CALCSOILPARS(NLAYER,NROOTLAYER,SOILWP,FRACWATER, &
                            ...
                            ZPD, NOTREES,IWATTABLAYER,ISIMWATTAB)   
      ! M. Christina, added ZBC, RZ, for gravitational potential calculation
...
! all layer below the water table are saturated with water (around 40% normaly)
      ...
      ! Calculate soil conductivity and soil-to-root resistance 
      !if using measured soil water content.
...
! Note that water is not actually taken up yet.
      CALL WATERUPTAKELAYER(SOILWP,SOILRRES1,SOILRRES2, &
                            ...
                            TOTESTEVAP,ZBC,RZ) 
      ! M. Christina, laythick 12/2012, ZBC,RZ, gravitational potential calculation 
...
!**********************************************************************
        ...
        SUBROUTINE WATBALLAY(IDAY,IHOUR,PPT,RUTTERB,RUTTERD,MAXSTORAGE, &
                              ...
                              IWATTABLAYER,ISIMWATTAB,PLATDRAIN,WATCAPIL,TOTLAI)
...
!            print*,FRACWATER(37:39),WATERGAINCAPIL(38)
!            pause
        ELSE 
            WATERGAINCAPIL = 0.0
            WATCAPIL = 0.0
...
!       Calculate drainage for each soil layer
        ...
        ENDDO
!        print*,WATERLOSS(37:39)
!        print*,WATERGAIN(37:39)
!        print*,WATERGAINCAPIL(37:39)
!        pause
        ...
        IF (ISIMWATTAB.EQ.0) THEN
            ...
            !DISCHARGE = 1000*WATERLOSS(NLAYER)
            DO K=1,nlayer
                DISCHARGE=DISCHARGE !+ 1000*LDRAIN(K)
            ENDDO
        ELSE
            ! water gain by the watertable
            !DISCHARGE = 1000 * WATERGAIN(MIN(NLAYER+1,IWATTABLAYER))  
!            DISCHARGE = 1000*WATERLOSS(IWATTABLAYER-1)
            DO K=1,IWATTABLAYER-1
                DISCHARGE=DISCHARGE + 1000*LDRAIN(K)
            ENDDO
        END IF
            !DISCHARGE = 1000 * WATERGAIN(NROOTLAYER+1)  ! recharge of the water table
...
!       Add and subtract gains and losses for each layer above the water table

        IF (ISIMWATTAB.EQ.0) THEN
            IWATTABLAYER = NLAYER+1  ! to avoid bug
        END IF
...
!            print*, J, WATERCONTENT, WATERGAIN(J),WATERLOSS(J),WATERGAINCAPIL(J)
!            pause
...
!**********************************************************************
      SUBROUTINE FINDSOILTK(IDAY,TAIRK, GAMSOIL, &
                            ...
                            THERMCOND1, LAYTHICK1, LAYTHICK2, POREFRAC1, &      
                            SOILWP1,DRYTHICK,TORTPAR,VIEWFACTOR)
      ! layerthick2 M. Christina 09/2013, proposition when 2 width layers differs
...
!**********************************************************************
      ...
      ! This actually should be the depth from the middle of 
      !layer 1 to the middle of layer 1, which
      ! only is LAYTHICK1 when 1 and 2 have the same thickness!!!
      ! Proposition of modification by M. Christina 09/2013
	  QC = -THERMCOND1 * (SOILTK - SOILTK2)/(0.5*LAYTHICK1+0.5*LAYTHICK2)   
...
!**********************************************************************
    ...
    IF(WATERCONTENT.EQ.0.) THEN
      SOILWPFUN = -999.0           
      ! bug mathias décembre 2012 pour éviter valeure 
      !-Infinity qui crée NA dans WEIGHTEDSMP
    ELSE 
      ...
      IF(RETFUNCTION.EQ.3) THEN
          ...
          ELSE IF (WATERCONTENT.GT.WS) THEN
              SOILWPFUN = -0.0001
          ELSE
              SOILWPFUN = - 1/ALPHARET * &
                  (((WS-WR)/(WATERCONTENT-WR))**(NRET/(NRET-1)) - 1)**(1/NRET) 
          END IF
      END IF   
    ENDIF
...
! Soil hydraulic con
!*      ductivity, using the Campbell (1974) equation.
...
!**********************************************************************
      ...
      USE maestcom
      IMPLICIT NONE
      INTEGER NROOTLAYER,USEMEASSW,NITER,I,J
      REAL LSOIL,MPAM,LA
      REAL SOILCOND(MAXSOILLAY),ROOTMASS(MAXSOILLAY),ROOTLEN(MAXSOILLAY)
...
! Hydraulic head for conversion (MPa / m)
      MPAM = 0.009807
       ...
       DO I=1,NROOTLAYER
              ...
              KSOIL = SOILCOND(I)  / (H2OVW * GRAV * 1E-03) ! Remark : MPAM = GRAV *1E-03
            ...
            ELSE
                    ...
                    ! Note : this component is calculated but not used (see wateruptakelayer proc). 
...
!print*,SOILR2(I), DEPTH, ROOTRESCONS, ROOTRESIST,LA
!print*, SOILR1(I),ROOTLEN(I)
!pause
       ...
       ENDDO
            ...
            !print*,'soilres',SOILR1(1:nrootlayer)
                !print*,'ROOTLEN',ROOTLEN(1:nrootlayer)
            !pause
...
!**********************************************************************
      ...
      SUBROUTINE WATERUPTAKELAYER(SOILWP,SOILRRES1,SOILRRES2, &
                                  ...
                                  FRACUPTAKE,TOTSOILRES,LAYTHICK, &
                                  TOTESTEVAP,ZBC,RZ) 
      ! M. Christina, added LAYTHICK 09/2012 and ZBC 09/2013
...
!**********************************************************************
        ...
        ! SOILRRES2 is commented out :
        ! don't use root-component of resistance (is part of plant resistance)
...
!                ESTEVAP(I)=(SOILWP(I)-MINROOTWP - 0.01*(TREEHEIGHT+DEPTH))/SOILRRES(I)   
                ESTEVAP(I)=(SOILWP(I)-MINROOTWP - 0.01*(DEPTH))/SOILRRES(I)   
!                ESTEVAP(I)=(SOILWP(I)-MINROOTWP)/SOILRRES(I)   
                ! Modification by M. Christina 09/2013,
                ! added gravimetric potential base on Duursma et al. 2007
...
!                WEIGHTEDSWP = WEIGHTEDSWP + (SOILWP(I)-0.01*(DEPTH))*ESTEVAP(I)
                WEIGHTEDSWP = WEIGHTEDSWP + (SOILWP(I)- 0.01*(DEPTH))/SOILRRES(I)
!                WEIGHTEDSWP = WEIGHTEDSWP + (SOILWP(I))/SOILRRES(I)
...
!            WEIGHTEDSWP = WEIGHTEDSWP / TOTESTEVAP       
            WEIGHTEDSWP = WEIGHTEDSWP / SUM(1/SOILRRES(1:NROOTLAYER))       
        ELSE
    ...
    !print*, TOTSOILRES,'tot'
    !print*,SOILRRES(1:NROOTLAYER)
    !pause
        ...
        IF(SUM(FRACUPTAKE2(1:NROOTLAYER)).GT.0)THEN
          FRACUPTAKE2 = FRACUPTAKE2 / SUM(FRACUPTAKE2(1:NROOTLAYER))  
...
!**********************************************************************
        ...
        REAL VPDPA,THROUGHFALL,RUTTERB,RUTTERD, CAN_STORE
        REAL CANOPY_STORE, SURFACE_WATERMM, EVAPSTORE
        REAL DRAINSTORE,X1,X2,HMIN,H1,EPS,DXSAV
        REAL CANSTORPREV,DELTASTORE,EXPDIF,TOTLAI
        REAL MAXSTORAGENEW
        ...
        !MAXSTORAGENEW = MAXSTORAGE*TOTLAI
        !MAXSTORAGENEW = 0.17*TOTLAI
        MAXSTORAGENEW = MAXSTORAGE * (1-THROUGHFALL)
        ...
        IF(CANOPY_STORE.LT.1E-6*(MAXSTORAGENEW))   CANOPY_STORE = 0.

        ! Store previous value of canstore:
        CANSTORPREV = CANOPY_STORE

!        ! Integrate the canopy storage diff. eq. (CANSTOR function).
        CAN_STORE = CANSTORPREV + (1- THROUGHFALL)*PPT
        IF(CAN_STORE.GT.MAXSTORAGENEW) THEN
            DRAINSTORE = CAN_STORE - MAXSTORAGENEW
            CAN_STORE = MAXSTORAGENEW
        END IF
        
        ! Wet evaporation rate (returns EVAPSTORE).
        CALL WETEVAP(WIND,ZHT,Z0HT,ZPD, &
                           PRESSPA,TAIRC,RNET, &
                           VPDPA,CAN_STORE,MAXSTORAGENEW, &
                           EVAPSTORE,PPT,THROUGHFALL)


        CANOPY_STORE = CAN_STORE - EVAPSTORE
        IF(EVAPSTORE.GT.CAN_STORE) THEN 
            EVAPSTORE = CAN_STORE
            CANOPY_STORE = 0.0
        ENDIF
        SURFACE_WATERMM = DRAINSTORE + THROUGHFALL*PPT
...
!**********************************************************************
        ...
        REAL EVAPSTORE,DRAINSTORE,EXPDIF
...
!        RUTTERA = LOG(RUTTERD) - RUTTERB*MAXSTORAGE*(1-EXPDIF)
!        RUTTERA = LOG(RUTTERD) - RUTTERB*MAXSTORAGE
        
!        IF(Y(1).GT.MAXSTORAGE)THEN
!        IF(Y(1).GT.(MAXSTORAGE*(1-EXPDIF)))THEN
...
!        ENDIF
        ...
        IF(Y(1).GT.MAXSTORAGE)THEN
            ! rate of drainage from store, mm t-1
            DRAINSTORE = Y(1) - MAXSTORAGE !+ REAL(EXP(RUTTERA + RUTTERB * MAXSTORAGE)*NUMMIN)
            Y(1) = MAXSTORAGE
        ELSE
            DRAINSTORE = 0 !REAL(EXP(RUTTERA + RUTTERB * Y(1))*NUMMIN)
        ENDIF
...
!**********************************************************************
        ...
        REAL POTEVAPMUMOL,POTENTIALEVAP,RATIO,THROUGHFALL
...
!        RATIO = MIN(1.,CANSTORE/(MAXSTORAGE/(1-EXPDIF)))
!        EVAPSTORE = POTENTIALEVAP * RATIO * (1-EXPDIF)
!print*,'can',canstore, maxstorage/(1-EXPDIF),evapstore
!pause
!        RATIO = MIN(1.,CANSTORE/(MAXSTORAGE))
        EVAPSTORE = POTENTIALEVAP * RATIO   ! car par m2 de feuille
!        EVAPSTORE = POTENTIALEVAP * RATIO * THROUGHFALL
...
!**********************************************************************
        ...
        REAL WATERGAIN(MAXSOILLAY), WATERLOSS(MAXSOILLAY)
        REAL SWCb,SWCa,SWPb,SWPa,KZI,SWCI(100),LTI,WCAPIL,ERRT
        INTEGER I
        REAL, EXTERNAL :: SOILCONDFUN
        REAL, EXTERNAL :: SOILWPFUN
        

        !Fraction of water above and below (above = the target layer)
        SWCb = FRACWATER(J+1)
        SWCa = FRACWATER(J)
        ERRT = 0.00001 !to avoid extrem case
        
        ! Soil water potential difference to determine if there is capilary risign
        SWP2 = SOILWPFUN(FRACWATER(J+1),PSIE(J+1),BPAR(J+1), &
             POREFRAC(J+1)+ERRT,ALPHARET(J+1),WS(J+1)+ERRT, &
             WR(J+1)-ERRT,NRET(J+1),RETFUNCTION)
        SWP1 = SOILWPFUN(FRACWATER(J),PSIE(J),BPAR(J), &
             POREFRAC(J)+ERRT,ALPHARET(J),WS(J)+ERRT,WR(J)-ERRT,NRET(J),RETFUNCTION)
        ...
        IF((DELTASWP.GT.1).AND.(J.GT.5)) THEN
            ...
            DO TIME = 1,SPERHR
                ...
                ! Gradient of soil water content assumed linear between the 2 layers
                SWCI(1) = SWCb
                    DO I=1,50   ! assumed very small layer of laythick/50 cm
                       SWCI(I+1) = (SWCb*((50-I)*2+1) + SWCa*((i-1)*2+1))/100
                    ENDDO
                    
                !!Calculation of capilary rising by integrating RIchards equation between the 2 layers
                WCAPIL=0.
                    DO I = 2,51
                        LTI = 0.5*(LAYTHICK(J)+LAYTHICK(J+1))/50
                        KZI = SOILCONDFUN( (SWCI(I-1)+SWCI(I))/2, KSAT(J),BPAR(J),POREFRAC(J)+ERRT, &
                                            WS(J)+ERRT,WR(J),ALPHARET(J),NRET(J),RETFUNCTION)
                        SWPb = SOILWPFUN(SWCI(I-1),PSIE(J),BPAR(J), &
                                         POREFRAC(J)+ERRT,ALPHARET(J),WS(J)+ERRT, &
                                         WR(J)-ERRT,NRET(J),RETFUNCTION)
                        SWPa = SOILWPFUN(SWCI(I),PSIE(J),BPAR(J), &
                                         POREFRAC(J)+ERRT,ALPHARET(J),WS(J)+ERRT, &
                                         WR(J)-ERRT,NRET(J),RETFUNCTION)
                        WCAPIL = WCAPIL - KZI * (1 + (SWPa-SWPb) * 1000000/(1000*9.81*LTI)) * LTI / &
                                        (0.5*(LAYTHICK(J)+LAYTHICK(J+1)))
                    ENDDO
                
                SWCLAY = SWCLAY + WCAPIL
            
            ENDDO
            WATERFLUXCAPIL = (SWCLAY - FRACWATER(J)) * LAYTHICK(J)
            UNSAT =(POREFRAC(J)-FRACWATER(J)+WATERGAIN(J)-WATERLOSS(J))*LAYTHICK(J) 
            WATERGAINCAPIL(J) = min(WATERFLUXCAPIL, UNSAT)  ! m d'H2O pet time step
...
! Alternatively, do water balance only based on the target trees 
      !(no scaling to stand).
