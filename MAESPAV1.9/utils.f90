...
!**********************************************************************
    ...
    ELSE
        IF (A.EQ.0.0) THEN
            IF (B.EQ.0.0) THEN
                QUADM = 0.0
                IF (C.NE.0.0) CALL SUBERROR('ERROR: CANT SOLVE QUADRATIC',&
                                            IWARN,0)
            ELSE
                QUADM = -C/B
            END IF
...
!**********************************************************************
    ...
    DO NSTP=1,MAXSTP
        ! Add as argument an array with all parameters. 
        !This array can then be passed between functions,
        ...
        CALL RKQS(Y,DYDX,NVAR,X,H,EPS,YSCAL,HDID,HNEXT,DERIVS,EXTRAPARS)  
            ! RAD change
        IF(HDID.EQ.H)THEN
            NOK=NOK+1
        ELSE
