...
!**********************************************************************
SUBROUTINE PSTRANSPIF(iday,ihour,RDFIPT,TUIPT,TDIPT,RNET,WIND,PAR,TAIR,TMOVE,&
                    CA,RH,VPD,VMFD,PRESS,JMAX25,&
                    ...
                    SMD1,SMD2,WC1,WC2,SOILDATA,SWPEXP,FSOIL,G0,D0L,GAMMA,VPDMIN,&
                    G1,GK,WLEAF,NSIDES,   &
                    VPARA,VPARB,VPARC,VFUN,SF,PSIV,ITERMAX,GSC,ALEAF,RD,ET,FHEAT, &
                    TLEAF,GBH,PLANTK,TOTSOILRES,MINLEAFWP,WEIGHTEDSWP,KTOT,&
...
!**********************************************************************
    ...
    IF(MODELGS.EQ.6)THEN
        ...
        CALL PSILFIND(RDFIPT,TUIPT,TDIPT,RNET,WIND,PAR,TAIR,TMOVE,CA,RH,VPD,VMFD,PRESS,JMAX25,&
                    ...
                    VPARA,VPARB,VPARC,VFUN,SF,PSIV,ITERMAX,GSC,ALEAF,RD,ET,FHEAT,TLEAF,GBH,&
                    PLANTK,TOTSOILRES,MINLEAFWP,  &
    ...
    CALL PSTRANSP(iday,ihour,RDFIPT,TUIPT,TDIPT,RNET,WIND,PAR,TAIR,TMOVE,CA,RH,VPD,VMFD,PRESS,JMAX25,&
                    ...
                    VPARA,VPARB,VPARC,VFUN,SF,PSIV,ITERMAX,GSC,ALEAF,RD,ET,FHEAT,TLEAF,GBH,&
                    PLANTK,TOTSOILRES,MINLEAFWP,   &
                    WEIGHTEDSWP,KTOT,HMSHAPE,PSILIN,PSIL,ETEST,CI,ISMAESPA,G02,G12,NEWTUZET)
!if(PSIL.LT.(-3.5)) THEN
!    print*, PSILIN, PSIL
!    print*, iday, ihour
!endif
...
!**********************************************************************
    ...
    ! modification TLEAF in TLEAF1 M. christina 01/2014
    ! Total boundary layer conductance for heat
    GBH = GBHU + GBHF
...
!    IF(ISMAESPA)THEN
    IF(ISMAESPA.AND.MODELGS.NE.6)THEN
     ...
     ENDIF
    IF(ISMAESPA.AND.MODELGS.EQ.6)THEN       ! Christina Mathias, there is no reason to use ETEST with the Tuzet model (because PSILMIN is not used)
...
!        GV = (GBV*GSV)/(GBV+GSV)
        !  Call Penman-Monteith equation
!        ET = PENMON(PRESS,SLOPE,LHV,RNET,VPD,GH,GV) *1E6
!        FHEAT = RNET - LHV*ET
!    ENDIF
...
!**********************************************************************
    ...
    ! MAESPA uses Emax approach, or Tuzet.
    ! MAESTRA calculates it various ways based on input data.
        ...
        IF(ISMAESPA)THEN
            IF(WSOILMETHOD.EQ.1.AND.MODELGS.NE.6)THEN
                ...
                IF(ETEST > EMAXLEAF)THEN
                    ...
                    ! Matter of choice? What happens when calculated GS < G0?
                    ! Is G0 a hard minimum or only in well-watered conditions?
...
!**********************************************************************
    ...
    REAL Cd, X, TOTLAI, ZPD2, TREEH, Z02, KH, ALPHA, Z0HT2
...
!        GBCANMS1 = WIND*(VONKARMAN**2)/(LOG((ZHT - ZPD)/Z0HT))**2
        GBCANMS1 = WIND*(VONKARMAN**2)/(LOG((ZHT - ZPD2)/Z02))**2
    ELSE
        GBCANMS1 = 0.0
    END IF
...
!    END IF
    ...
    KH = (VONKARMAN**2) * (TREEH - ZPD2) * WIND / log((ZHT - ZPD2)/Z02)

    !Aerodynamic conductance soir-air below canopy
    GBCANMS2 = ALPHA * KH / ( TREEH * exp(ALPHA) * (exp(-ALPHA * Z0HT2/TREEH)  - & 
                exp(-ALPHA * (ZPD2+Z02) / TREEH) ) )
...
!**********************************************************************
    ...
    IF(SETFSOIL.EQ.0)THEN
        CALL SUBERROR('Error calculating FSOIL in CALCFSOIL. Check parameters.',&
                        IFATAL,IOERROR)
    END IF
...
SUBROUTINE PSILFIND(RDFIPT,TUIPT,TDIPT,RNET,WIND,PAR,TAIR,TMOVE,CA,RH,VPD,VMFD,PRESS,JMAX25,&
                    ...
                    SMD1,SMD2,WC1,WC2,SOILDATA,SWPEXP,FSOIL,G0,D0L,GAMMA,VPDMIN,&
                    G1,GK,WLEAF,NSIDES,   &
                    VPARA,VPARB,VPARC,VFUN,SF,PSIV,ITERMAX,GSC,ALEAF,RD,ET,&
                    FHEAT,TLEAF,GBH,PLANTK,TOTSOILRES,MINLEAFWP,  &
                    WEIGHTEDSWP,HMSHAPE,PSILIN,ETEST,iday,ihour,G02,G12,NEWTUZET)
...
!**********************************************************************
        ...
        INTEGER EXTRAINT(10)
          ...
          ISMAESPA = .TRUE. ! modification M. CHristina 02/2014
          !ISMAESPA = .FALSE.
!        iday = -1
!        ihour = -1
        ...
        CALL PSTRANSP(iday,ihour,RDFIPT,TUIPT,TDIPT,RNET,WIND,PAR,TAIR,TMOVE,& 
             CA,RH,VPD,VMFD,PRESS,JMAX25, &
             IECO,EAVJ,EDVJ,DELSJ,VCMAX25,EAVC,EDVC,DELSC,TVJUP,TVJDN,THETA,AJQ,RD0, &
             Q10F,K10F,RTEMP,DAYRESP,TBELOW,MODELGS,WSOILMETHOD,EMAXLEAF,SOILMOISTURE,    &
             SMD1,SMD2,WC1,WC2,SOILDATA,SWPEXP,FSOIL,G0,D0L,GAMMA,VPDMIN,G1,GK,WLEAF,NSIDES,   &
             VPARA,VPARB,VPARC,VFUN,SF,PSIV,ITERMAX,GSC,ALEAF,RD,ET,FHEAT,  &
             TLEAF,GBH,PLANTK,TOTSOILRES,MINLEAFWP, WEIGHTEDSWP,KTOT,HMSHAPE,& 
             PSILIN,PSIL,ETEST,CI,ISMAESPA,G02,G12,NEWTUZET)
