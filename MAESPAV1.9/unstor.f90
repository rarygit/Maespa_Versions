...
!**********************************************************************
SUBROUTINE INPUTUSSTR(NOUSPOINTS,X0,Y0,GRDAREAI,XLU,YLU,ZLU,USLAI,NOFUDATES,&
                        DATESFU,HTUS,&
...
!**********************************************************************
    ...
    IF ((IOERROR.NE.0).OR.(NOUSPOINTS.EQ.0)) THEN
        CALL SUBERROR('ERROR: MISSING CONTROL INFO IN USTOREY FILE',IFATAL,IOERROR)
    END IF
...
!**********************************************************************
    ...
    IF (IOERROR.NE.0) CALL SUBERROR('INPUT ERROR: MISSING UNDERSTOREY BEWDY PARAMS',&
                                    IFATAL,IOERROR)
        JMAXN25I = JMAXN25
        VCMAXN25I = VCMAXN25
        UNMINI = UNMIN
    ...
    IF (IOERROR.NE.0)CALL SUBERROR('INPUT ERROR: MISSING UNDERSTOREY JMAXPARS',&
                                    IFATAL,IOERROR)
    REWIND (UFILE)
    READ (UFILE,VCMAXPARS,IOSTAT = IOERROR)
    IF (IOERROR.NE.0)CALL SUBERROR('INPUT ERROR: MISSING UNDERSTOREY VCMAXPARS',&
                                    IFATAL,IOERROR)
    ...
    ELSE
        ...
        IF (IOERROR.NE.0)CALL SUBERROR('INPUT ERROR: MISSING UNDERSTOREY GS PARS',&
                                        IFATAL,IOERROR)
        GSBG0U = G0
        GSBG1U = G1
        IF (G1.LT.0.0) CALL SUBERROR('ERROR IN GS PARAMETERS: G1 MUST BE > 0',&
                                      IFATAL,0)
    END IF
    ...
    IF (IOERROR.NE.0) CALL SUBERROR('WARNING: MISSING UNDERSTOREY RD PARS',& 
                                    IWARN,IOERROR)
    RD0I = RD
    RDKI = RDK
    RDTI = RDT
