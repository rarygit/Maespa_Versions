...
!! The function 'surface' finds relative radial distance to crown
!    surface as function of relative height
...
!! BM 7/03: Moved this within loop to correct error with JLEAF = 0, 
!    NOAGEC > 1, NOAGEP > 1
!        DO 100 IPT = 1,NUMPNT
!          DLT(IPT) = 0.0
!          DO 60 IAGE = 1,NOAGEC
...
! calculate relative radial distance - differs in odd & even layers
! The function 'surface' finds relative radial distance to crown surface
! as function of relative height
        IF(MOD(LAYER,2).EQ.1) THEN
          CORR = SURFACE(CZ(IPQ),JSHAPE)
        ELSE
          ! Multiply by cos 45°
...
! 1. Calculate co-ordinates (XL,YL,ZL) and volume (VL) for each grid point.
    ...
    DO LAYER = 1, NOLAY      ! for each layer
        ...
        ! calculate relative radial distance - differs in odd & even layers
        ! The function 'surface' finds relative radial distance to
        ! crown surface as function of relative height
        IF(MOD(LAYER,2).EQ.1) THEN
          CORR = SURFACE(CZ(IPQ),JSHAPE)
        ELSE
      ...
      DO 900 IPT = 1,NUMPNT
        ...
        DO 500 J = 1,NZEN
          ...
          DO 400 K = 1,NAZ
            ...
            !! DEXT is output!! DEXTANG=0 not used
...
300         IFLAG = 2
            ...
            !! DEXT is output!!  !modification mathias mars 2013
...
! Z1 in a canopy of thickness (Z1 + Z2). (Norman & Welles 1983)
     ...
     Z2 = -1    ! Move from Z2=0 ? Initialize Z2 (to avoid a bug,
                ! suggested by Alejandro Morales)
...
!            print*,'slope',slope,(PID2-ZENITH),BEXT
...
!       SLA is the sunlit leaf area associated with the grid point.
      ...
      ENDIF
!            print*,'pos',xpt,ypt,zpt
!            print*,'btemp',BEXT,S,S1, BTEMP
...
! ADUM(J) is the ratio of upwards to downwards diffuse flux (eqn 3.6.13,
! Norman 1979)
      TLAY2 = TLAYER*TLAYER
      DO 700 J = 2,JTOT
...
!**********************************************************************
    ...
    ! Error corrected from previous version, which here had D(JTOT-1) in
    ! lieu of D(1). V. small effect.
    DO J = 2,LAYERI
        U(J) = U(J-1)*EXPDIF + FF
    END DO
...
SUBROUTINE ABSRAD(IPT,IWAVE, &
        ...
        DFLUX,BFLUX,SCATFX,DEXTT)   ! dextt guerric 04/14
...
!**********************************************************************
    ...
    IF (ZEN.GT.1.56) THEN   ! limite l'angle zenithal à 89.4° pour éviter
                            ! certains bug ! mars 2013 mathias
        BMULT = 0.0
    END IF
