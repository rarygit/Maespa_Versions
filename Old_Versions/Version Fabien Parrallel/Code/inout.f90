...
!**********************************************************************
    ...
    IMPLICIT NONE
    INTEGER LEN1,IOERROR,IWATFILE,IUSTFILE, KEEPZEN  !, ISUNLA   ! deleted M. Christina 27/11/12
...
!**********************************************************************
    ...
    IF (IOFORMAT .EQ. 0) THEN
        ...
        IF (ISUNLA.EQ.1) THEN
            WRITE (USUNLA,461)    ! Added output, M. Christina 27/11/12
        END IF
...
!            WRITE (UHRLY,717)
    ...
    461   format('DOY   Hour    Tree    IPT    SUNLA   AREA    BEXT    FBEAM   ZEN  ABSRPPAR  ABSRPNIR ABSRPTH BFPAR DFPAR BFNIR DFNIR DFTHR SCLOSTPAR SCLOSTNIR SCLOSTTH DOWNTH PARABV NIRABV THRABV')   ! Added output, M. Christina 27/11/12
   ...
   ! 717 FORMAT('ELMAX: Canopy maximum leaf transpiration rate (mmol m-2 s-1)')
    ...
    726 FORMAT('ZEN: Zenithal angle (rad)')                      ! Added output M. Chrsitina March 2013
...
!**********************************************************************
    ...
    CLOSE(USUNLA)  ! Added output M. Christina 27/11/12
    CLOSE(STDIN)
    CLOSE(UTREES)
    CLOSE(USTR)
    CLOSE(UPHY)
    ...
    IF (IOWATBAL.EQ.1 .AND. IOFORMAT .EQ. 0) THEN
        ...
        CLOSE(UWATDAY)     
        CLOSE(UWATLAY)     
        CLOSE(USWPLAY) ! Added output M. Christina December 2012     
        CLOSE(USOILT)      
        CLOSE(UWATUPT)
...
!**********************************************************************
    ...
    CALL READROOTPARS(UWATPARS,ROOTRESFRAC, ROOTRADTABLE,ROOTDENSTABLE,ROOTMASSTOTTABLE, NROOTLAYER,FRACROOT, &
        LAYTHICK, ROOTBETA,DATESROOT,NOROOTDATES)    ! Multi-date possibility for root parameters, M. Christina December 2012
...
!             
!    FRACROOT = FRACROOT / FRACSUM          ! Modification M. Christina December 2012, now calculated in INTERPOLATEDIST
    
    ! Root cross-sectional area (m2)
!        ROOTXSECAREA = PI*ROOTRAD**2        ! Modification M. Christina December 2012, now calculated in INTERPOLATEDIST
         ROOTXSECAREA = 0
...
SUBROUTINE READROOTPARS(UFILE, ROOTRESFRACI,ROOTRADTABLEI,ROOTDENSTABLEI, &  
                         ...
                         LAYTHICK,ROOTBETA,DATESROOTI,NOROOTDATES)! Added rootBeta to fix bug M. Christina
!**********************************************************************
...
!**********************************************************************
    ...
    REAL X0INT,Y0INT,XMAXINT,YMAXINT ! Mathias June 2013
    
    ! Read in number of trees & number of target tree
    CALL READPLOT(UTREES, X0, Y0, XMAX, YMAX, NOALLTREES,XSLOPE, YSLOPE, BEAR, SHADEHT, STOCKING, IPLOTSHAPE,X0INT,Y0INT,XMAXINT,YMAXINT)  ! Mathias June 2013
    PLOTAREA = (XMAXINT - X0) * (YMAXINT - Y0)  ! Mathias June 2013
        ...
        DO ITAR=1,NOTARGETS
            ...
            IF (IOFORMAT .EQ. 0) THEN
                WRITE (UHRLY,500) IDAY,ITREE,ISPEC,IHOUR,                                       &
                                    ...
                                    ZEN(IHOUR),AZ(IHOUR)                    !Added output, M. Christina March 2013                
500             FORMAT (I7,1X,3(I4,1X),3(F12.5,1X),16(F12.5,1X),2(F12.5,1X))
            ELSE IF (IOFORMAT .EQ. 1) THEN
...
SUBROUTINE OUTPUTWATBAL(IDAY,IHOUR,NROOTLAYER,NLAYER,          &
                              ...
                              SCLOSTTOT,SOILWP,FRACAPAR) !Added SOILWP, M. Christina, December 2012
...
!**********************************************************************
    ...
    REAL SOILTEMP(MAXSOILLAY), SOILWP(MAXSOILLAY) !Added M. Christina, December 2012
    ...
    IF (IOWATBAL .EQ. 1 .AND. IOFORMAT .EQ. 0) THEN
        ...
        ! write swp by layer
        WRITE (USWPLAY, 521) SOILWP(1:60) !Added M. Christina, December 2012
    ...
    ELSE IF (IOWATBAL .EQ. 1 .AND. IOFORMAT .EQ. 1) THEN
        ...
        ! write swp by layer
        WRITE (USWPLAY) SOILWP(1:25) !Added M. Christina, December 2012
...
SUBROUTINE READPLOT(UFILE, X0I, Y0I, XMAXI, YMAXI, NOALLTREESI, &
                    ...
                    X0INTI,Y0INTI,XMAXINTI,YMAXINTI) ! mathias June 2013
...
!**********************************************************************
    ...
    USE maestcom
    IMPLICIT NONE
    INTEGER UFILE,IOERROR,IPLOTSHAPE,PLOTSHAPE,NOTREES,NOALLTREESI,AREANOTREES
    
    REAL X0I, Y0I, XMAXI, YMAXI, XSLOPEI, YSLOPEI, BEARI
    REAL SHADEHTI, STOCKING, X0,Y0,XMAX,YMAX,XSLOPE,YSLOPE,BEARING,SHADEHT
    REAL INTXMAX,INTYMAX,INTX0,INTY0,XMAXINTI,YMAXINTI,X0INTI,Y0INTI    ! mathias June 2013
    
    NAMELIST /PLOT/ X0,Y0,XMAX,YMAX,NOTREES,XSLOPE,YSLOPE,BEARING,SHADEHT,PLOTSHAPE,INTXMAX,INTYMAX,INTX0,INTY0 !,AREANOTREES  modification M. Christina  June 2013 to resolve bug in case usestand=0
    ...
    END IF
    NOALLTREESI = NOTREES
   ! NOTREES2 = AREANOTREES  ! modification M. Christina  June 2012 to resolve bug in case usestand=0
    
    IF (IPLOTSHAPE.EQ.0) THEN
        STOCKING = NOTREES/(XMAX*YMAX)       ! modification M. Christina, June 2013 to resolve bug in case usestand=0
    ELSE
...
!**********************************************************************
    ...
    IF (AVGANG.LT.0.0.AND.ELP.LT.0.0) THEN        
        ...
        DO IALP = 1,NALPHAI
            ALPHA(IALP) = (IALP-0.5)*DALPHA     ! Fix bug calculating alpha, M. Christina February 2013
        END DO
        INDEX = 1
        DO IDATE = 1,NODATESLIA
            DO IANG = 1,NALPHAI                     
...
SUBROUTINE INTERPOLATEDIST(IDAY,ISTART,FRACROOTTABLE,NOROOTDATES,DATESROOT,FRACROOT,NROOTLAYER, &
    ...
    IF(ISMAESPA)THEN
        ...
        FRACSUM = SUM(FRACROOT(1:NROOTLAYER))
        ! Modification, M. Christina December 2012
