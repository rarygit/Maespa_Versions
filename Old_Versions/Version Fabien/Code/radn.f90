...
! Calculate transmittances (this is the old function TRANSD).
  ...
  300       IFLAG = 2
            ...
            CALL TREDST(IFLAG,IPROG,IRAD,DZDN,DAZ, &
               XL(IPT),YL(IPT),ZL(IPT),RX,RY,RZ,DXT,DYT,DZT, &
               FOLT,ZBC,JLEAFT,BPTT,NOAGECT,PROPCT,JSHAPET,SHAPET, &
               DEXTT(1:MAXT,J),NT,SL,SL1,DEXT(J),DEXTANGT,DEXTANG)  !! DEXT is output!!  Modification to use DEXTANG, previously no calculated, M. Christina, March 2013
...
! Here for LAI < 5, we try DLAI = 0.1
      !DLAI = REAL(IFIX(TOTLAI/5.0)+1)*0.1 !! Modification M. Christina 12/2012
    DLAI=0.01
...
! Z1 in a canopy of thickness (Z1 + Z2). (Norman & Welles 1983)
      ...
      ! Initialize Z2 (to avoid a bug, suggested by Alejandro Morales).
      Z2 = 0
...
!**********************************************************************
    ...
    REAL DIFDN(MAXP,3),DIFUP(MAXP,3),SCLOST(MAXP,3)
    REAL DOWNTH(MAXP)
    REAL U(MAXP),D(MAXP) ! glm replace 101 by MAXP
...
!**********************************************************************
    ...
    IF (ZEN.GT.1.56) THEN   ! Zenithal angle threshold  to avoid excessive Beam flux when cos(ZEN) is reaching 0, M. Christina March 2013
        BMULT = 0.0
    END IF
